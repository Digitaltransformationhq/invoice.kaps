...
alter table public.auditors add column if not exists company_id uuid;
alter table public.auditors add column if not exists email text;
alter table public.auditors add column if not exists full_name text;
alter table public.auditors add column if not exists password_hash text;
alter table public.auditors add column if not exists is_active boolean default true;
alter table public.auditors add column if not exists last_login timestamptz;
alter table public.auditors add column if not exists created_by uuid;
alter table public.auditors add column if not exists created_at timestamptz default now();
...
begin
  if not exists (
    ...
    where conrelid = 'public.auditors'::regclass
      and contype  = 'f'
      and confrelid = 'public.companies'::regclass
  ) then
    alter table public.auditors
      add constraint auditors_company_id_fkey
...
begin
  if not exists (
    ...
    alter table public.auditors
      add constraint auditors_created_by_fkey
...
begin
  if not exists (
    ...
    where conrelid = 'public.auditors'::regclass
      and contype  = 'u'
      and pg_get_constraintdef(oid) like '%(company_id, email)%'
  ) then
    begin
      alter table public.auditors
        add constraint auditors_company_email_key unique (company_id, email);
...
begin
  ...
  loop
    execute format(
      'alter table public.auditors alter column %I drop not null',
      v_col.column_name
    );
  end loop;
end $$;
...
begin
  ...
  if v_has_name then
    insert into public.auditors (
      id, company_id, email, full_name, name, password_hash,
      is_active, last_login, created_by, created_at, updated_at
    )
    select
      id, company_id, email, full_name, full_name, password_hash,
      is_active, last_login, created_by, created_at, updated_at
