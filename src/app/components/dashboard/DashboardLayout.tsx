            ...
            <div className="flex items-center gap-3 flex-1">
              ...
              <div className="hidden sm:flex items-center flex-1 w-full max-w-3xl" ref={searchRef}>
                <div className="relative w-full">
                  ...
                  <input
                    type="text"
                    placeholder="Search invoices, customers, GSTIN, phone, email…"
                    value={searchQuery}
                    onChange={(e) => {
                    ...
                    className="w-full pl-10 pr-4 h-9 border border-input bg-input-background rounded-lg text-[13px] placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent/40 transition"
                  />
                  ...
                  {showSearchResults && (isSearching || searchResults.invoices.length > 0 || searchResults.customers.length > 0) && (
                    <div className="absolute top-full mt-2 w-full bg-white border border-border rounded-lg shadow-lg max-h-96 overflow-y-auto z-50">
                          ...
                          <div className="p-2">
                            {searchResults.invoices.map((invoice) => (
                              <Link
                                ...
                                className="flex items-center justify-between px-3 py-2 hover:bg-violet-50 dark:hover:bg-violet-500/10 rounded transition-colors"
                              >
                                <div>
                                  <div className="text-sm font-semibold text-violet-700 dark:text-violet-300">{invoice.invoiceNumber}</div>
                      ...
                      {searchResults.customers.length > 0 && (
                        <div className="border-t border-violet-100 dark:border-violet-400/15">
                          <div className="px-4 py-2 text-[11px] font-semibold tracking-wider uppercase text-violet-700 dark:text-violet-300 bg-violet-100 dark:bg-violet-500/15">
                            Customers
                          </div>
                          <div className="p-2">
                            {searchResults.customers.map((customer) => (
                              ...
                              >
                                <div className="min-w-0">
                                  <div className="text-sm font-semibold text-violet-700 dark:text-violet-300">{customer.name}</div>
